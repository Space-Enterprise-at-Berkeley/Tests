...
class Presentation(object):
    ...
    def get_content(self):
        self.presentation_obj = self.slides_service.presentations().get(presentationId=self.presentation_id).execute()
    ...
    def send_edits(self):
        if self.edits:
            ...
            response = self.slides_service.presentations().batchUpdate(presentationId=self.presentation_id, body=body).execute()
...
class Slide(object):
    ...
    def _insert_text(self, element_id, text):
        return {
            'insertText': {
                'objectId': element_id,
                'insertionIndex': 0,
    ...
    def _delete_text(self, element_id):
        return {
            'deleteText': {
                'objectId': element_id,
                'textRange': {
...
class TextSlide(Slide):
    ...
    def __init__(self, raw, pres):
        super().__init__(raw, pres)
        # assert len(raw['pageElements']) == 3, "expect TextSlide to have 3 page elements"
        self.title, self.text1, self.text2 = raw['pageElements']
        
    def set_title(self, txt):
        self.pres.add_edit(self._delete_text(self.title['objectId']))
